...
require_once __DIR__ . '/../config.php'; // Per costanti globali, percorsi, funzioni di log, e inclusione di functions_url.php
require_once __DIR__ . '/db_connection.php';    // Per la connessione al DB
require_once __DIR__ . '/functions_csrf.php';     // Per i token CSRF
// functions_settings.php è necessario per get_site_setting() usato nelle email
if (!function_exists('get_site_setting')) { 
    if (file_exists(__DIR__ . '/functions_settings.php')) {
        require_once __DIR__ . '/functions_settings.php';
    } else {
        if(function_exists('log_error')) log_error("CRITICO: functions_settings.php non trovato.", __FILE__, __LINE__);
    }
}
// functions_url.php dovrebbe essere già incluso da config.php
if (!function_exists('generate_url')) {
    if (file_exists(__DIR__ . '/functions_url.php')) { 
        require_once __DIR__ . '/functions_url.php';
    } elseif (defined('PROJECT_ROOT') && file_exists(PROJECT_ROOT . '/includes/functions_url.php')) { 
        require_once PROJECT_ROOT . '/includes/functions_url.php';
    } else {
        if(function_exists('log_error')) log_error("CRITICO: functions_url.php non trovato.", __FILE__, __LINE__);
    }
}


// Dichiarazioni 'use' per PHPMailer
use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\SMTP;
use PHPMailer\PHPMailer\Exception;

// Inclusione condizionale dei file di PHPMailer
...
if (!function_exists('update_user_password')) {
    function update_user_password($user_id, $new_password) {
        $conn = get_db_connection(); 
        if (empty($new_password) || strlen($new_password) < 8) { return ['success' => false, 'message' => "La nuova password deve essere di almeno 8 caratteri."]; }
        $new_password_hash = password_hash($new_password, PASSWORD_DEFAULT); 
        if ($new_password_hash === false) { log_error("Errore hashing password ID: {$user_id}", __FILE__, __LINE__); return ['success' => false, 'message' => "Errore sicurezza."]; }
        $stmt = $conn->prepare("UPDATE users SET password_hash = ?, requires_password_change = FALSE, validation_token = NULL, validation_token_expires_at = NULL, initial_password_setup_token = NULL, initial_password_setup_token_expires_at = NULL, password_reset_token = NULL, password_reset_token_expires_at = NULL WHERE id = ?");
        if (!$stmt) { log_error("DB Error (prep update_user_password): " . $conn->error, __FILE__, __LINE__); return ['success' => false, 'message' => "Errore DB."]; }
        $stmt->bind_param("si", $new_password_hash, $user_id);
        if ($stmt->execute()) { 
            if ($stmt->affected_rows > 0) { log_activity("Password aggiornata/impostata e token resettati per ID: {$user_id}", $user_id); }
            else { log_activity("Tentativo update password ID: {$user_id}, no rows (pwd identica o flag già ok, tokens resettati comunque se query eseguita).", $user_id); }
            $stmt->close(); 
            if (isset($_SESSION['user_id']) && $_SESSION['user_id'] == $user_id) { unset($_SESSION['force_password_change']); unset($_SESSION['is_setting_initial_password']); } 
            return ['success' => true, 'message' => "Password aggiornata."];
        } else { log_error("DB Error (exec update_user_password): " . $stmt->error, __FILE__, __LINE__); $stmt->close(); return ['success' => false, 'message' => "Errore DB aggiornamento password."]; }
    }
}
 ...
 */
if (!function_exists('request_email_change')) {
    function request_email_change($user_id, $new_email, $current_password) {
        ...
        if ($stmt_update->execute()) {
            ...
            $validation_link = generate_url('validate_email_change', ['token' => $token]);
 ...
 */
if (!function_exists('verify_email_change_token')) {
 ...
 */
if (!function_exists('send_user_email_validation_link')) {
    function send_user_email_validation_link($user_email, $username, $token) {
        ...
        if (!function_exists('send_custom_email') || !function_exists('generate_url')) { log_error("send_custom_email o generate_url non disponibile per validazione utente."); return false; }
        $validation_link = generate_url('validate_user_email', ['token' => $token]);
        $site_name = get_site_setting('site_name', (defined('SITE_NAME') ? SITE_NAME : 'Il Nostro Servizio'));
 ...
 */
if (!function_exists('send_admin_new_user_notification')) {
    function send_admin_new_user_notification($admin_email, $new_username, $new_user_email) {
        ...
        if (!function_exists('send_custom_email') || !function_exists('generate_url')) { log_error("send_custom_email o generate_url non disponibile per notifica admin."); return false; }
        ...
        $admin_link = generate_url('admin_pending_users');
        $body_html = "<p>Nuovo utente registrato:</p><ul><li>Username: ".htmlspecialchars($new_username)."</li><li>Email: ".htmlspecialchars($new_user_email)."</li></ul><p>Approva nel pannello admin: <a href='{$admin_link}'>Valida Utenti</a></p>";
...
if (!function_exists('send_password_reset_link')) {
    function send_password_reset_link($user_email, $username, $token) {
        ...
        if (!function_exists('send_custom_email') || !function_exists('generate_url')) {
            log_error("Funzioni necessarie (send_custom_email o generate_url) non disponibili per send_password_reset_link.", __FILE__, __LINE__);
            return false;
        }

        // Genera l'URL per il reset password. Il token verrà aggiunto come query string da generate_url.
        $reset_link = generate_url('reset_password', ['token' => $token]); 
        
        // LOGGA L'URL GENERATO PRIMA DI INVIARLO
        log_activity("[PwdResetEmailLink] Link generato per email: " . $reset_link);
 ...
 */
if (!function_exists('verify_user_email_validation_token')) {
 ...
 * Con logging di debug per tracciare il flusso email.
 */
if (!function_exists('admin_validate_user_account')) {
    function admin_validate_user_account($user_id_to_validate, $assigned_role = 'User', $is_active_status = TRUE) {
        ...
        if ($assigned_role !== 'Admin' && $assigned_role !== 'User') { 
            log_error("[AdminValidate] Ruolo non valido: {$assigned_role} per UserID: {$user_id_to_validate}");
            return ['success' => false, 'message' => "Ruolo non valido specificato."]; 
        }
        ...
        if (!$stmt_user_check) { 
            log_error("[AdminValidate] DB Err (prep user check): ".$conn->error." per UserID: {$user_id_to_validate}"); 
            return ['success'=>false,'message'=>'Errore DB recupero dati utente.']; 
        }
        $stmt_user_check->bind_param("i", $user_id_to_validate); 
        $stmt_user_check->execute();
        $user_current_data = $stmt_user_check->get_result()->fetch_assoc(); 
        $stmt_user_check->close();

        if (!$user_current_data) { 
            log_error("[AdminValidate] Utente non trovato per UserID: {$user_id_to_validate}");
            return ['success' => false, 'message' => 'Utente non trovato per la validazione.']; 
        }
        log_activity("[AdminValidate] Dati UserID {$user_id_to_validate}: EmailVal=".($user_current_data['is_email_validated']?'Sì':'No').", PwdHash='".($user_current_data['password_hash']===''?'VUOTO':'IMPOSTATO')."', ReqPwdChg=".($user_current_data['requires_password_change']?'Sì':'No'), $_SESSION['user_id'] ?? 'Sistema');

        if ($assigned_role === 'User' && $is_active_status === TRUE && !($user_current_data['is_email_validated'] ?? false)) {
            log_warning("[AdminValidate] Attivazione UserID {$user_id_to_validate} (ruolo User) fallita: email non validata.", $_SESSION['user_id'] ?? 'Sistema');
            return ['success' => false, 'message' => "L'email dell'utente non è stata ancora validata. Impossibile attivare l'account come 'User'."];
        }
        
        $initial_password_setup_token = NULL;
        $initial_password_setup_token_expires_at = NULL;
        $needs_initial_password_setup = (
            $is_active_status === TRUE && 
            ($user_current_data['password_hash'] === '' || $user_current_data['password_hash'] === NULL) && 
            ($user_current_data['requires_password_change'] == TRUE) 
        );
        log_activity("[AdminValidate] UserID {$user_id_to_validate}: NeedsPwdSetup? " . ($needs_initial_password_setup ? 'Sì' : 'No'), $_SESSION['user_id'] ?? 'Sistema');

        if ($needs_initial_password_setup) {
            $initial_password_setup_token = bin2hex(random_bytes(32));
            $initial_password_setup_token_expires_at = date('Y-m-d H:i:s', time() + (48 * 3600)); 
            log_activity("[AdminValidate] UserID {$user_id_to_validate}: Token setup generato: " . $initial_password_setup_token, $_SESSION['user_id'] ?? 'Sistema');
        }
        ...
        $sql_update = "UPDATE users SET 
                            ...
                            role = ?, 
                            initial_password_setup_token = ?, 
                            initial_password_setup_token_expires_at = ? 
                       WHERE id = ? AND requires_admin_validation = TRUE";
        
        $stmt = $conn->prepare($sql_update);
        if (!$stmt) { 
            log_error("[AdminValidate] DB Error (prepare admin validate user): " . $conn->error . " SQL: " . $sql_update, __FILE__, __LINE__);
            return ['success' => false, 'message' => "Errore database durante la preparazione della validazione."]; 
        }
        
        $active_int = $is_active_status ? 1 : 0;
        $types_string = "isssi"; 
        
        $stmt->bind_param($types_string, 
            $active_int, 
            $assigned_role, 
            $initial_password_setup_token, 
            $initial_password_setup_token_expires_at, 
        ...
        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) {
                ...
                log_activity("[AdminValidate] Admin (ID: {$admin_id_acting}) ha validato l'utente ID: {$user_id_to_validate} con ruolo: {$assigned_role}, attivo: {$is_active_status}", $admin_id_acting);
                
                if ($is_active_status && function_exists('send_account_activated_email')) { 
                    log_activity("[AdminValidate] Chiamata send_account_activated_email per UserID: {$user_id_to_validate}, Email: {$user_current_data['email']}, Token: ".($initial_password_setup_token ?? 'Nessuno'), $admin_id_acting);
                    $email_sent_result = send_account_activated_email($user_current_data['email'], $user_current_data['username'], $initial_password_setup_token); 
                    log_activity("[AdminValidate] Risultato invio email attivazione per UserID {$user_id_to_validate}: " . ($email_sent_result ? 'Successo' : 'Fallimento'), $admin_id_acting);
                }
                return ['success' => true, 'message' => "Utente validato con successo."];
            } else { 
                $stmt->close(); 
                log_warning("[AdminValidate] Nessun utente aggiornato per UserID: {$user_id_to_validate}. Forse già validato o email non verificata?", $_SESSION['user_id'] ?? 'Sistema');
                return ['success' => false, 'message' => "Nessun utente aggiornato. L'utente potrebbe essere già stato validato, oppure l'email non è ancora verificata (se richiesta per l'attivazione)."]; 
            }
        } else {
            log_error("[AdminValidate] DB Error (execute admin validate user): " . $stmt->error, __FILE__, __LINE__); 
            $stmt->close();
            return ['success' => false, 'message' => "Errore database durante la validazione dell'utente."];
        }
    }
}
 ...
 */
if (!function_exists('send_account_activated_email')) {
    function send_account_activated_email($user_email, $username, $initial_password_token = null) {
        ...
        if (!function_exists('send_custom_email') || !function_exists('generate_url')) { 
            log_error("[SendActivatedEmail] Funzioni necessarie (send_custom_email o generate_url) non disponibili per invio a {$user_email}.", __FILE__, __LINE__);
            return false; 
        }
        $site_name = get_site_setting('site_name', (defined('SITE_NAME') ? SITE_NAME : 'Il Nostro Servizio'));
        ...
        if ($initial_password_token) {
            $setup_link = generate_url('change_password', ['action' => 'setup', 'token' => $initial_password_token]);
            $body_html .= "<p>Per completare l'attivazione e accedere, per favore imposta la tua password cliccando sul seguente link (valido per 48 ore):</p>";
            $body_html .= "<p><a href='{$setup_link}'>{$setup_link}</a></p>";
            log_activity("[SendActivatedEmail] Email per {$user_email} includerà il link di setup password: {$setup_link}");
        } else {
            $login_link = generate_url('login');
            $body_html .= "<p>Ora puoi accedere al sito utilizzando le tue credenziali: <a href='{$login_link}'>{$login_link}</a></p>";
            log_activity("[SendActivatedEmail] Email per {$user_email} NON includerà un link di setup password (token non fornito o utente non ne necessita).");
        }
        $body_html .= "<p>Grazie,<br>Il Team di " . htmlspecialchars($site_name) . "</p>";
        $email_sent = send_custom_email($user_email, $subject, $body_html);
        if (!$email_sent) { log_error("[SendActivatedEmail] Fallito invio effettivo email di attivazione a {$user_email} per utente {$username}.", __FILE__, __LINE__); }
        else { log_activity("[SendActivatedEmail] Email di attivazione (send_custom_email ha restituito ".($email_sent ? 'true':'false').") inviata a {$user_email}."); }
        return $email_sent;
    }
}
 ...
 */
if (!function_exists('verify_initial_password_setup_token')) {
 ...
 */
if (!function_exists('verify_password_reset_token')) {
    function verify_password_reset_token($token) {
        ...
        if (!$stmt) { log_error("DB Error (prep verify_password_reset_token): " . $conn->error, __FILE__, __LINE__); return false; }
        $stmt->bind_param("ss", $token, $current_time); $stmt->execute(); $user = $stmt->get_result()->fetch_assoc(); $stmt->close();
 ...
 */
if (!function_exists('get_pending_validation_users')) {
    function get_pending_validation_users() {
        ...
        if ($result) { while ($row = $result->fetch_assoc()) $users[] = $row; $result->free(); } else log_error("Errore get_pending: " . $conn->error, __FILE__, __LINE__);
        return $users;
    }
}
 ...
 */
if (!function_exists('admin_reject_pending_user')) {
...
if (!function_exists('require_login')) {
     ...
     * Se l'utente è loggato ma deve cambiare password, lo reindirizza alla pagina di cambio password,
     * a meno che non sia già su quella pagina.
     * @param string|null $page_to_redirect_to La pagina a cui reindirizzare se non loggato (usa generate_url se null).
     */
    function require_login($page_to_redirect_to = null) { 
        if (!is_logged_in()) {
            $_SESSION['flash_message'] = 'Devi effettuare il login per accedere a questa pagina.';
            $_SESSION['flash_type'] = 'warning';
            
            $login_page_generated_path = function_exists('generate_url') ? generate_url('login', [], false) : 'login.php';
            $current_request_uri_path_only = strtok($_SERVER['REQUEST_URI'], '?');
            $normalized_current_path = $current_request_uri_path_only;

            // Normalizza il path corrente rimuovendo la RewriteBase se presente
            $rewrite_base_to_check = defined('REWRITE_BASE_FOR_PHP') ? REWRITE_BASE_FOR_PHP : '';
            if (!empty($rewrite_base_to_check) && $rewrite_base_to_check !== '/') {
                if (strpos($normalized_current_path, $rewrite_base_to_check) === 0) {
                    $normalized_current_path = substr($normalized_current_path, strlen($rewrite_base_to_check));
                }
            }
            $normalized_current_path = '/' . ltrim($normalized_current_path, '/');
            
            // Normalizza il path della pagina di login generato
            $normalized_login_path = $login_page_generated_path; // generate_url con false dovrebbe già dare path relativo a RewriteBase o root
             if (defined('REWRITE_BASE_FOR_PHP') && REWRITE_BASE_FOR_PHP !== '/') {
                if (strpos($normalized_login_path, REWRITE_BASE_FOR_PHP) === 0) { // Rimuovi se generate_url lo include
                    $normalized_login_path = substr($normalized_login_path, strlen(REWRITE_BASE_FOR_PHP));
                }
            }
            $normalized_login_path = '/' . ltrim(strtok($normalized_login_path, '?'), '/');


            if ($normalized_current_path !== $normalized_login_path) {
                $_SESSION['redirect_after_login'] = $_SERVER['REQUEST_URI'];
            }

            $redirect_url = $page_to_redirect_to ?: (function_exists('generate_url') ? generate_url('login') : 'login.php');
            header('Location: ' . $redirect_url);
            exit;
        }

        // Check per cambio password forzato
        if (isset($_SESSION['force_password_change']) && $_SESSION['force_password_change'] === true) {
            $is_on_change_password_page = false;
            if (function_exists('generate_url')) {
                // generate_url('change_password', [], false) dovrebbe restituire il path "pulito" relativo
                // alla RewriteBase, es. 'cambia-password'
                $change_password_clean_route = generate_url('change_password', [], false); 
                
                $current_request_path = strtok($_SERVER['REQUEST_URI'], '?'); // Path completo, es. /fm/cambia-password

                // Estrai la parte dell'URI dopo la RewriteBase
                $path_after_base = $current_request_path;
                if (defined('REWRITE_BASE_FOR_PHP') && REWRITE_BASE_FOR_PHP !== '/') {
                    $base = rtrim(REWRITE_BASE_FOR_PHP, '/'); // es. /fm
                    if (strpos($path_after_base, $base) === 0) {
                        $path_after_base = substr($path_after_base, strlen($base));
                    }
                }
                $path_after_base = ltrim($path_after_base, '/'); // es. 'cambia-password' o 'admin/utenti-pendenti'

                log_activity("[RequireLoginCheck] Confronto per cambio password: CurrentPathSegment='{$path_after_base}', TargetCleanRoute='{$change_password_clean_route}'");

                if ($path_after_base === $change_password_clean_route) {
                    $is_on_change_password_page = true;
                }
            ...
            log_activity("[RequireLoginCheck] IsOnOnChangePwdPage: " . ($is_on_change_password_page ? 'SI':'NO'));

            if (!$is_on_change_password_page) {
                $_SESSION['flash_message']='Devi cambiare la tua password prima di continuare.';
                $_SESSION['flash_type']='warning';
                log_activity("[RequireLoginRedirect] Reindirizzamento forzato a cambia password.");
                header('Location: '.(function_exists('generate_url') ? generate_url('change_password', ['forced'=>1]) : 'change_password.php?forced=1'));
                exit;
            }
        }
    }
...
if (!function_exists('require_admin')) {
    function require_admin() { 
        require_login(); 
        if (!is_admin()) { 
            $_SESSION['flash_message'] = 'Accesso negato. Area riservata agli amministratori.'; 
            $_SESSION['flash_type'] = 'danger'; 
            header('Location: '.(function_exists('generate_url') ? generate_url('home') : 'index.php')); 
            exit; 
        } 
    }
}
...
if (!function_exists('send_custom_email')) {
    function send_custom_email($to, $subject, $body_html, $body_text = '') {
        if (!class_exists('PHPMailer\PHPMailer\PHPMailer')) { 
            log_error("PHPMailer non è caricato. L'email a {$to} non è stata inviata.", __FILE__, __LINE__); 
            return false; 
        }
        ...
        try {
            ...
            if (defined('SITE_EMAIL_FROM') && filter_var(SITE_EMAIL_FROM, FILTER_VALIDATE_EMAIL)) { 
                $from_email = SITE_EMAIL_FROM; 
            } elseif (isset($_SERVER['SERVER_NAME'])) {
                $potential_from = 'noreply@' . $_SERVER['SERVER_NAME'];
                if (filter_var($potential_from, FILTER_VALIDATE_EMAIL, FILTER_FLAG_HOSTNAME)) {
                    $from_email = $potential_from;
                } else {
                    $from_email = 'noreply@localhost.localdomain'; 
                    log_warning("SERVER_NAME ('{$_SERVER['SERVER_NAME']}') non è un hostname valido per l'indirizzo FROM dell'email, usando fallback: {$from_email}");
                }
            }
            if (!filter_var($from_email, FILTER_VALIDATE_EMAIL)) { 
                log_error("Indirizzo FROM non valido ({$from_email}) specificato per l'invio dell'email. Email a {$to} non inviata.", __FILE__, __LINE__); 
                return false; 
            }
            $mail->setFrom($from_email, $site_name_email); 
            $mail->addAddress($to);
            $mail->isHTML(true); 
            $mail->CharSet = 'UTF-8'; 
            $mail->Subject = $subject; // Lascia che PHPMailer gestisca l'encoding dell'oggetto
            $mail->Body    = $body_html;
            $mail->AltBody = $body_text ?: strip_tags($body_html);
            if ($mail->send()) { 
                log_activity("Email (tramite mail()) inviata con successo a {$to}, oggetto: {$subject}"); 
                return true; 
            } else { 
                log_error("Errore funzione mail() inviando a {$to}: {$mail->ErrorInfo}", __FILE__, __LINE__); 
                return false; 
            }
        } catch (Exception $e) { 
            log_error("PHPMailer Exception (mail()) inviando email a {$to}: {$mail->ErrorInfo}. Messaggio Exception: " . $e->getMessage(), __FILE__, __LINE__); 
            return false; 
        }
    }
}
